...
const MARKUP_PCT = 0; // No markup — profit comes from VTPass commission only

// ── Sandbox billersCode swap (per VTPass official sandbox docs) ──
// In sandbox, VTPass only accepts specific hardcoded test numbers.
// Swap in test numbers for VTPass calls; Firestore stores user's real input.
function sandboxBillers(real, type) {
  if (VTPASS_ENV !== 'sandbox') return real;
  if (type === 'prepaid')   return '1111111111111'; // electricity prepaid
  if (type === 'postpaid')  return '1010101010101'; // electricity postpaid
  if (type === 'tv')        return '1212121212';    // all TV providers
  if (type === 'airtime')   return '08011111111';   // airtime
  if (type === 'data')      return '08011111111';   // data
  if (type === 'jamb')      return '0123456789';    // JAMB profile ID
  // waec / waec-registration / betting — use real value (no billersCode restriction)
  return real;
}
...
app.post('/api/vtu/airtime', async (req, res) => {
  ...
  // Call VTPass async
  let vtRes;
  try { vtRes = await callVTPass({ request_id:requestId, serviceID:AIRTIME_SVC[networkCode], amount:numAmount, phone: sandboxBillers(phone, 'airtime') }); }
  catch (e) {
...
app.post('/api/vtu/data', async (req, res) => {
  ...
  let vtRes;
  try { vtRes = await callVTPass({ request_id:requestId, serviceID:DATA_SVC[networkCode], billersCode:sandboxBillers(phone,'data'), variation_code:variationCode, amount:numAmount, phone:sandboxBillers(phone,'data') }); }
  catch (e) {
...
app.get('/api/vtu/plans/:networkCode', async (req, res) => {
  ...
  try {
    ...
    const plans = raw.map(p => ({
      ...
      // No markup — user pays exact VTPass price. Profit = VTPass commission only.
      variation_amount: Number(p.variation_amount),
    }));
...
app.post('/api/vtu/verify-meter', async (req, res) => {
  ...
  try {
    const r = await axios.post(`${VTPASS_BASE}/merchant-verify`,
      { billersCode: sandboxBillers(String(meterNumber).trim(), meterType||'prepaid'), serviceID, type: meterType || 'prepaid' },
...
app.post('/api/vtu/verify-smartcard', async (req, res) => {
  ...
  try {
    const r = await axios.post(`${VTPASS_BASE}/merchant-verify`,
      { billersCode: sandboxBillers(String(smartcard).trim(), 'tv'), serviceID },
      { headers:VTPASS_HEADERS_POST, timeout:25000 });
    const vtData = r.data || {};
...
app.get('/api/vtu/service-plans/:serviceID', async (req, res) => {
  ...
  // Only serve cache if it has actual plans (never cache empty results)
  if (_plansCache[key] && _plansCache[key].length > 0)
    return res.json({ plans: _plansCache[key] });

  try {
    const resp = await axios.get(
      `${VTPASS_BASE}/service-variations?serviceID=${serviceID}`,
      { headers: VTPASS_HEADERS_GET, timeout: 20000 });

    const vtData = resp.data || {};
    // VTPass spells it 'varations' (typo in their API) — handle both
    const raw = vtData.content?.varations || vtData.content?.variations || [];
    const code = vtData.response_description || vtData.code || '';

    console.log('[service-plans]', serviceID, '→', raw.length, 'plans. code:', code);

    if (raw.length === 0) {
      // VTPass returned zero plans — likely not whitelisted or auth issue
      console.warn('[service-plans] EMPTY response for', serviceID, '— full response:', JSON.stringify(vtData).slice(0, 300));
      return res.status(503).json({
        error: 'No plans returned from provider. Please whitelist this product on your VTPass dashboard.',
        vtCode: code,
        plans: []
      });
    }
    ...
    console.error('[service-plans] error:', serviceID, '|', detail);
    return res.status(500).json({ error: `VTPass error: ${e.message}`, plans: [] });
  }
});

/* ---------- GET /api/vtu/debug (NO AUTH — remove before going live) ---------- */
app.get('/api/vtu/debug', async (req, res) => {
  const svc = req.query.svc || 'dstv';
  const envStatus = {
    VTPASS_ENV:    process.env.VTPASS_ENV    || '(not set — defaulting to sandbox)',
    VTPASS_BASE:   VTPASS_BASE,
    API_KEY_SET:   !!(process.env.VTPASS_API_KEY)    ? `YES (${String(process.env.VTPASS_API_KEY).slice(0,6)}...)` : 'NO — MISSING!',
    SECRET_KEY_SET:!!(process.env.VTPASS_SECRET_KEY) ? `YES (${String(process.env.VTPASS_SECRET_KEY).slice(0,6)}...)` : 'NO — MISSING!',
    PUBLIC_KEY_SET:!!(process.env.VTPASS_PUBLIC_KEY) ? `YES (${String(process.env.VTPASS_PUBLIC_KEY).slice(0,6)}...)` : 'NO — MISSING!',
  };
  try {
    const url = `${VTPASS_BASE}/service-variations?serviceID=${svc}`;
    const resp = await axios.get(url, { headers: VTPASS_HEADERS_GET, timeout: 15000 });
    const vtData = resp.data || {};
    const raw = vtData.content?.varations || vtData.content?.variations || [];
    return res.json({
      env: envStatus,
      serviceID: svc,
      url,
      vtpass_http_status: resp.status,
      vtpass_response_code: vtData.code || vtData.response_description || '(none)',
      plans_count: raw.length,
      raw_response: vtData,
    });
  } catch (e) {
    return res.status(500).json({
      ...
      serviceID: svc,
      error: e.message,
      vtpass_status: e.response?.status,
      vtpass_body: e.response?.data,
    });
  }
});
...
app.post('/api/vtu/electricity', async (req, res) => {
  ...
  try {
    vtRes = await callVTPass({
      ...
      billersCode: sandboxBillers(meterNumber, meterType),
      variation_code:meterType, amount:numAmount,
...
app.post('/api/vtu/tv', async (req, res) => {
  ...
  try {
    vtRes = await callVTPass({
      request_id:requestId, serviceID, billersCode: sandboxBillers(smartcard, 'tv'),
...
app.post('/api/vtu/education', async (req, res) => {
  ...
  let vtRes;
  try {
    // JAMB needs billersCode = JAMB Profile ID (sandbox: 0123456789)
    // WAEC / waec-registration do NOT use billersCode (phone-only)
    const eduPayload = {
      request_id:requestId, serviceID,
      variation_code:variationCode, amount:numAmount,
      phone: phone || '08011111111',
    ...
    if (serviceID === 'jamb') {
      // billersCode = profileID sent from frontend, sandboxed automatically
      const rawProfileId = req.body.profileID || req.body.billersCode || '0123456789';
      eduPayload.billersCode = sandboxBillers(String(rawProfileId).trim(), 'jamb');
    }
...
/* =====================================================================
   STORE / WHITE-LABEL RESELLER ROUTES
   ===================================================================== */

/* ---------- Subdomain middleware — detects slug.globalstasks.name.ng ---------- */
function getStoreSlugFromHost(req) {
  const host = (req.headers.host || '').toLowerCase().split(':')[0];
  const base = 'globalstasks.name.ng';
  if (host.endsWith('.' + base)) {
    const slug = host.slice(0, host.length - base.length - 1);
    if (slug && /^[a-z0-9]{2,10}$/.test(slug)) return slug;
  }
  return null;
}
...
app.post('/api/store/create', async (req, res) => {
  ...
  const { slug, businessName, logoUrl, template, services } = req.body || {};

  // Validate slug
  if (!slug || !/^[a-z0-9]{2,10}$/.test(slug))
    return res.status(400).json({ status:'fail', message:'Slug must be 2–10 lowercase letters/numbers.' });
  if (!businessName || businessName.trim().length < 2)
    return res.status(400).json({ status:'fail', message:'Business name required.' });
  if (!services || Object.keys(services).filter(k => services[k].enabled).length === 0)
    return res.status(400).json({ status:'fail', message:'Select at least one service.' });

  // Validate prices — each enabled service price must be >= vtpassPrice
  for (const [svcId, svc] of Object.entries(services)) {
    if (!svc.enabled) continue;
    if (!svc.vtpassPrice || svc.price < svc.vtpassPrice)
      return res.status(400).json({ status:'fail', message:`Price for ${svcId} must be at least ₦${svc.vtpassPrice}.` });
  }

  const CREATION_FEE = 2000;
  const storeRef  = dbAdmin.collection('stores').doc(slug);
  const userRef   = dbAdmin.collection('users').doc(uid);
  const txnRef    = dbAdmin.collection('Transaction').doc();
  const now       = admin.firestore.FieldValue.serverTimestamp();

  try {
    await dbAdmin.runTransaction(async tx => {
      // Check slug not taken
      const existing = await tx.get(storeRef);
      if (existing.exists) throw Object.assign(new Error('That URL is already taken. Choose another.'), {code:409});
      ...
      if (bal < CREATION_FEE)
        throw Object.assign(new Error(`Insufficient balance. You need ₦${CREATION_FEE} to create a store.`), {code:400});

      // Deduct fee
      tx.update(userRef, { balance: bal - CREATION_FEE });
      ...
      tx.set(txnRef, {
        ...
        amount: CREATION_FEE,
        status: 'success',
        description: `Store creation fee — ${slug}.globalstasks.name.ng`,
        timestamp: now,
      });
    });

    return res.json({ status:'success', slug, url:`https://${slug}.globalstasks.name.ng` });
  } catch (e) {
    return res.status(e.code || 500).json({ status:'fail', message: e.message });
  }
});

/* ---------- GET /api/store/mine ---------- */
app.get('/api/store/mine', async (req, res) => {
  let decoded;
  try { decoded = await verifyIdTokenFromHeaderOrBody(req); }
  catch (e) { return res.status(401).json({ status:'fail', message: e.message }); }
  const uid = decoded.uid;
  ...
  try {
    const snap = await dbAdmin.collection('stores').where('merchantUid','==',uid).get();
    const stores = snap.docs.map(d => ({ slug: d.id, ...d.data() }));
...
/* ---------- GET /api/store/config/:slug (public) ---------- */
app.get('/api/store/config/:slug', async (req, res) => {
  const { slug } = req.params;
  try {
    const doc = await dbAdmin.collection('stores').doc(slug).get();
...
/* ---------- POST /api/store/update/:slug ---------- */
app.post('/api/store/update/:slug', async (req, res) => {
  let decoded;
  try { decoded = await verifyIdTokenFromHeaderOrBody(req); }
  catch (e) { return res.status(401).json({ status:'fail', message: e.message }); }
  const uid = decoded.uid;
  ...
  try {
    const doc = await dbAdmin.collection('stores').doc(slug).get();
    if (!doc.exists) return res.status(404).json({ status:'fail', message:'Store not found.' });
    if (doc.data().merchantUid !== uid) return res.status(403).json({ status:'fail', message:'Not your store.' });

    const { businessName, logoUrl, template, services, status } = req.body || {};
    ...
    if (services) {
      ...
      for (const [svcId, svc] of Object.entries(services)) {
        if (!svc.enabled) continue;
        if (svc.price < svc.vtpassPrice)
          return res.status(400).json({ status:'fail', message:`Price for ${svcId} must be ≥ ₦${svc.vtpassPrice}.` });
      }
      update.services = services;
    }
    if (status === 'active' || status === 'inactive') update.status = status;

    await dbAdmin.collection('stores').doc(slug).update(update);
    return res.json({ status:'success' });
  } catch (e) {
    return res.status(500).json({ status:'fail', message: e.message });
  }
...
/* ---------- GET /api/store/transactions/:slug ---------- */
app.get('/api/store/transactions/:slug', async (req, res) => {
  let decoded;
  try { decoded = await verifyIdTokenFromHeaderOrBody(req); }
  catch (e) { return res.status(401).json({ status:'fail', message: e.message }); }
  const uid = decoded.uid;
  ...
  try {
    const storeDoc = await dbAdmin.collection('stores').doc(slug).get();
    if (!storeDoc.exists) return res.status(404).json({ status:'fail', message:'Store not found.' });
    if (storeDoc.data().merchantUid !== uid) return res.status(403).json({ status:'fail', message:'Not your store.' });

    const snap = await dbAdmin.collection('store_transactions')
      .where('storeSlug','==',slug)
      .orderBy('timestamp','desc')
      .limit(50)
      .get();
    const txns = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    return res.json({ status:'success', transactions: txns });
  } catch (e) {
    return res.status(500).json({ status:'fail', message: e.message });
  }
});

/* ---------- POST /api/store/purchase/:slug (customer buys from storefront) ---------- */
app.post('/api/store/purchase/:slug', async (req, res) => {
  const { slug } = req.params;
  const { serviceID, variationCode, planName, merchantPrice, phone, paystackRef } = req.body || {};

  if (!serviceID || !variationCode || !merchantPrice || !phone || !paystackRef)
    return res.status(400).json({ status:'fail', message:'Missing required fields.' });

  // 1. Load store
  const storeDoc = await dbAdmin.collection('stores').doc(slug).get();
  if (!storeDoc.exists || storeDoc.data().status !== 'active')
    return res.status(404).json({ status:'fail', message:'Store not found or inactive.' });
  const store = storeDoc.data();
  const svc = store.services[serviceID];
  if (!svc || !svc.enabled)
    return res.status(400).json({ status:'fail', message:'Service not available.' });
  ...
  try {
    ...
    const psData = psResp.data?.data;
    if (!psData || psData.status !== 'success')
      return res.status(400).json({ status:'fail', message:'Payment not verified.' });
    paidNaira = Number(psData.amount) / 100;
    if (Math.abs(paidNaira - merchantPrice) > 1)
      return res.status(400).json({ status:'fail', message:'Payment amount mismatch.' });
  } catch (e) {
    return res.status(500).json({ status:'fail', message:'Could not verify payment.' });
  }

  // 3. Check for duplicate Paystack ref
  const psDocRef = dbAdmin.collection('store_payments').doc(paystackRef);
  const psDoc = await psDocRef.get();
  if (psDoc.exists) return res.status(400).json({ status:'fail', message:'Payment already used.' });

  const vtpassPrice = svc.vtpassPrice;
  const merchantProfit = paidNaira - vtpassPrice;
  const requestId = makeVTRequestId();
  const storeRef = dbAdmin.collection('stores').doc(slug);
  const merchantUserRef = dbAdmin.collection('users').doc(store.merchantUid);
  const storeTxnRef = dbAdmin.collection('store_transactions').doc(requestId);
  const merchantTxnRef = dbAdmin.collection('Transaction').doc();
  const now = admin.firestore.FieldValue.serverTimestamp();

  // 4. Save pending record + lock Paystack ref
  await dbAdmin.batch()
    .set(psDocRef, { paystackRef, storeSlug: slug, requestId, createdAt: now })
    .set(storeTxnRef, {
      storeSlug: slug, merchantUid: store.merchantUid,
      serviceID, variationCode, planName: planName || variationCode,
      customerPhone: phone, vtpassPrice, merchantPrice: paidNaira,
      merchantProfit, paystackRef, status:'processing',
      requestId, timestamp: now,
    })
    .commit();

  // Respond immediately — process async
  res.json({ status:'success', requestId, message:'Processing your purchase…' });
  ...
  try {
    const eduPayload = {
      ...
      amount: vtpassPrice,
      phone: VTPASS_ENV === 'sandbox' ? '08011111111' : phone,
      quantity: 1,
    };
    // JAMB needs billersCode
    if (serviceID === 'jamb') eduPayload.billersCode = VTPASS_ENV === 'sandbox' ? '0123456789' : phone;
    vtRes = await callVTPass(eduPayload);
  } catch (e) {
    console.error('[Store purchase] VTPass error:', e.message);
    // Refund — we need to return money. For now mark failed; merchant doesn't lose (Paystack already received)
    await dbAdmin.collection('store_transactions').doc(requestId).update({
      status:'failed', failReason:'VTPass error — ' + e.message, updatedAt: now
    });
    return;
  }

  const code = vtRes?.code;
  const eduPin = vtRes?.purchased_code || vtRes?.content?.transactions?.pin || '';
  const vtRef = vtRes?.content?.transactions?.transactionId || requestId;

  if (code === '000') {
    const b = dbAdmin.batch();
    b.update(storeTxnRef, { status:'success', eduPin, vtpassRef:vtRef, updatedAt:now });
    // Credit merchant profit to their Globals wallet + log transaction
    if (merchantProfit > 0) {
      b.update(merchantUserRef, { balance: admin.firestore.FieldValue.increment(merchantProfit) });
      b.set(merchantTxnRef, {
        userId: store.merchantUid, type:'StoreProfit',
        amount: merchantProfit, status:'success',
        description: `Store profit — ${slug} — ${planName || serviceID}`,
        storeSlug: slug, timestamp: now,
      });
      b.update(storeRef, {
        totalTransactions: admin.firestore.FieldValue.increment(1),
        totalRevenue: admin.firestore.FieldValue.increment(merchantProfit),
        totalCustomers: admin.firestore.FieldValue.increment(1),
      });
    }
    await b.commit();
    console.log(`[Store] ✅ ${slug} — ${serviceID} — profit ₦${merchantProfit}`);
  } else if (code === '099') {
    await dbAdmin.collection('store_transactions').doc(requestId).update({ status:'pending', vtpassRef:vtRef, updatedAt:now });
  } else {
    await dbAdmin.collection('store_transactions').doc(requestId).update({
      status:'failed', failReason: vtRes?.response_description || 'VTPass failed', updatedAt:now
    });
  }
});


/* ---------- GET /api/store/order/:requestId (storefront polling) ---------- */
app.get('/api/store/order/:requestId', async (req, res) => {
  try {
    const doc = await dbAdmin.collection('store_transactions').doc(req.params.requestId).get();
    if (!doc.exists) return res.json({ status:'pending', order:null });
    return res.json({ status:'success', order:{ id:doc.id, ...doc.data() }});
  } catch (e) {
    return res.status(500).json({ status:'fail', message: e.message });
  }
});

/* ---------- Subdomain store serving ---------- */
app.use((req, res, next) => {
  const slug = getStoreSlugFromHost(req);
  if (slug && !req.path.startsWith('/api/')) {
    const storePage = require('path').join(__dirname, 'public', 'store.html');
    if (require('fs').existsSync(storePage)) return res.sendFile(storePage);
  }
  next();
});
