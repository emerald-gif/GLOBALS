...
// ── Sandbox billersCode swap ──
// In sandbox, VTPass only accepts specific test numbers.
// We swap in the correct test number for VTPass calls, but store
// the user's real input in Firestore for display purposes.
function sandboxBillers(real, type) {
  if (VTPASS_ENV !== 'sandbox') return real;
  if (type === 'prepaid')  return '1111111111111';
  if (type === 'postpaid') return '1010101010101';
  if (type === 'tv')       return '1212121212';
  if (type === 'airtime')  return '08011111111';
  if (type === 'data')     return '08011111111';
  return real; // betting, education — use real
}
