...
async function callVTPass(payload) {
  const r = await axios.post(`${VTPASS_BASE}/pay`, payload, { auth: VTPASS_AUTH, timeout: 35000 });
  return r.data;
}
...
app.get('/api/vtu/plans/:networkCode', async (req, res) => {
  ...
  try {
    const resp = await axios.get(
      ...
      { auth: VTPASS_AUTH, timeout: 15000 }
    );
...
app.post('/api/vtu/requery', async (req, res) => {
  ...
  try {
    const r = await axios.post(`${VTPASS_BASE}/requery`, { request_id: requestId }, { auth: VTPASS_AUTH, timeout: 15000 });
