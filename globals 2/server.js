...
app.post("/api/request-withdrawal", async (req, res) => {
  try {
    ...
    await dbAdmin.runTransaction(async (tx) => {
      ...
      // Sync Ludo balance collection
      tx.set(adminDb.collection('balance').doc(userId), { amount: balance - amtNum, uid: userId }, { merge: true });
      tx.set(withdrawRef, {
        userId,
        accNum,
        bankName,
...
app.post("/api/verify-payment", async (req, res) => {
  try {
    ...
    await dbAdmin.runTransaction(async (tx) => {
      ...
      tx.set(balRef, { amount: newBalance, uid }, { merge: true });
...
app.get('/api/vtu/plans/:networkCode', async (req, res) => {
  ...
  try {
    ...
    const raw = resp.data?.content?.varations || [];
...
app.post('/api/ludo/claim-win', verifyToken, async (req, res) => {
  ...
  try {
    ...
    await adminDb.runTransaction(async tx => {
      ...
      // Credit winner — update BOTH Ludo balance collection and main wallet
      const balRef = adminDb.collection('balance').doc(uid);
...
app.post('/api/ludo/forfeit', verifyToken, async (req, res) => {
  ...
  try {
    ...
    await adminDb.runTransaction(async tx => {
      ...
      tx.update(adminDb.collection('users').doc(winnerUid), {
        balance: admin.firestore.FieldValue.increment(room.winAmount)
      });
      // Log transaction
      const txRef = adminDb.collection('ludo_transactions').doc();
      tx.set(txRef, {
...
app.post('/api/vtu/verify-meter', async (req, res) => {
  ...
  try {
    const r = await axios.post(`${VTPASS_BASE}/merchant-verify`,
      { billersCode:meterNumber, serviceID, type:meterType },
      { headers:VTPASS_HEADERS_POST, timeout:20000 });
    return res.json(r.data);
  } catch (e) { return res.status(500).json({ status:'fail', message:e.message }); }
});
...
app.post('/api/vtu/verify-smartcard', async (req, res) => {
  ...
  try {
    const r = await axios.post(`${VTPASS_BASE}/merchant-verify`,
      { billersCode:smartcard, serviceID },
      { headers:VTPASS_HEADERS_POST, timeout:20000 });
    return res.json(r.data);
  } catch (e) { return res.status(500).json({ status:'fail', message:e.message }); }
});
...
app.post('/api/vtu/tv', async (req, res) => {
  ...
  try {
    vtRes = await callVTPass({
      ...
      phone:phone||smartcard, subscription_type:'change',
    });
  } catch (e) {
...
app.post('/api/vtu/betting', async (req, res) => {
  ...
  const uid = decoded.uid;
  const { serviceID, customerID, amount, customerName, pin } = req.body || {};
  ...
  try {
    vtRes = await callVTPass({
      ...
      billersCode:customerID, amount:numAmount, phone:customerID,
    });
  } catch (e) {
...
app.post('/api/vtu/education', async (req, res) => {
  ...
  try {
    vtRes = await callVTPass({
      request_id:requestId, serviceID,
      billersCode:phone||'08000000000',
      variation_code:variationCode, amount:numAmount,
      phone:phone||'08000000000', quantity:Number(quantity||1),
    });
  } catch (e) {
