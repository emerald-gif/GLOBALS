...
function initReferralTab() {
  ...
  auth.onAuthStateChanged(async (user) => {
    ...
    // "See all invites" — show/hide after first 5
    const allCards = document.querySelectorAll("#referralList > div");
    const seeAllBtn = document.getElementById("seeAllInvitesBtn");
    ...
    window.toggleAllInvites = function() {
      ...
      const cards = document.querySelectorAll("#referralList > div");
      cards.forEach((c, i) => { if (i >= 5) c.style.display = showingAll ? "" : "none"; });
      const btn = document.getElementById("seeAllInvitesBtn");
      if (btn) btn.textContent = showingAll ? "Show less" : "See all invites";
    };
