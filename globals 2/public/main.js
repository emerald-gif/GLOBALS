...
/* ===== Globals: Navigation + Visibility + Perf guards (DROP THIS AT END OF main.js) ===== */
(function () {
  if (window.__globalsPerfPatched) return;
  window.__globalsPerfPatched = true;

  const DEFAULT_TAB = 'dashboard';
  const persistKey = 'globals:lastActiveTab';
  ...
  // --- wrap existing activateTab / switchTab so they push state (unless disabled by popstate) ---
  const _origActivate = window.activateTab;
  const _origSwitch = window.switchTab;

  function pushTabState(tab, replace = false) {
    try {
      const url = '#' + tab;
      const state = { tab };
      ...
      console.warn('pushTabState failed', e);
    }
  }

  if (typeof _origActivate === 'function') {
    window.activateTab = function (tabId, ...rest) {
      const suppressed = window.__globalsDisablePush === true;
      try { _origActivate.call(this, tabId, ...rest); } catch (e) { console.error(e); }
      if (!suppressed) pushTabState(tabId, false);
    };
  }
  ...
  if (typeof _origSwitch === 'function') {
    window.switchTab = function (tabId, ...rest) {
      const suppressed = window.__globalsDisablePush === true;
      try { _origSwitch.call(this, tabId, ...rest); } catch (e) { console.error(e); }
      if (!suppressed) pushTabState(tabId, false);
    };
  }

  // popstate -> restore tab, but avoid pushing a new state when we call the handlers
  window.addEventListener('popstate', function (ev) {
    const candidate = (ev.state && ev.state.tab) || (location.hash ? location.hash.replace('#', '') : null) || sessionStorage.getItem(persistKey) || DEFAULT_TAB;
    window.__globalsDisablePush = true;
    try {
      if (typeof _origActivate === 'function') _origActivate(candidate);
      else if (typeof _origSwitch === 'function') _origSwitch(candidate);
      else {
        document.querySelectorAll('.tab-section').forEach(s => s.classList.add('hidden'));
        const el = document.getElementById(candidate);
        if (el) el.classList.remove('hidden');
      }
    } catch (e) { console.error('popstate activate error', e); }
    // tiny timeout then re-enable pushing
    setTimeout(() => { window.__globalsDisablePush = false; }, 60);
  }, false);

  // On first load, restore tab from hash/sessionStorage (and replaceState so history is consistent)
  document.addEventListener('DOMContentLoaded', function () {
    const fromHash = location.hash ? location.hash.replace('#', '') : null;
    const initial = fromHash || sessionStorage.getItem(persistKey) || DEFAULT_TAB;
    try { history.replaceState({ tab: initial }, '', '#' + initial); } catch (e) {}
    if (typeof _origActivate === 'function') safeCall(_origActivate, initial);
    else if (typeof _origSwitch === 'function') safeCall(_origSwitch, initial);
    else {
      document.querySelectorAll('.tab-section').forEach(s => s.classList.add('hidden'));
  ...
  // --- Performance helpers: pause/resume heavy listeners when page is hidden/visible ---
  function pauseHeavyWork() {
    // stop transaction realtime listener (your code exposes stopTransactionsListener)
    try { if (typeof stopTransactionsListener === 'function') stopTransactionsListener(); } catch (e) { console.warn(e); }

    // stop any safe balance controller
    try { if (window.__balanceCtrl && typeof window.__balanceCtrl.stop === 'function') window.__balanceCtrl.stop(); } catch (e) { /* ignore */ }

    // stop swiper autoplay (tries common ways)
    try {
      // If Swiper attaches instance to element.swiper
      document.querySelectorAll('.swiper, .swiper-container').forEach(el => {
        const i = el.swiper || el.__swiper;
        if (i && i.autoplay && typeof i.autoplay.stop === 'function') {
          try { i.autoplay.stop(); } catch (e) { /* ignore */ }
        }
      });
      // Also global Swiper.instances if present
      if (window.Swiper && Array.isArray(window.Swiper.instances)) {
        window.Swiper.instances.forEach(s => { s?.autoplay?.stop && s.autoplay.stop(); });
      }
    } catch (e) { /* ignore */ }

    // Pause Chart.js animations (Chart.getChart for v3+)
    try {
      if (window.Chart && typeof Chart.getChart === 'function') {
        document.querySelectorAll('canvas').forEach(c => {
          try { const chart = Chart.getChart(c); if (chart && typeof chart.stop === 'function') chart.stop(); } catch (e) { /* ignore */ }
        });
      }
    } catch (e) { /* ignore */ }

    // Add a CSS class to dramatically reduce animations/transitions
    document.body.classList.add('app-paused');
  }

  function resumeHeavyWork() {
    // schedule resumption idly to avoid blocking the main thread immediately
    const resume = () => {
      try { if (typeof initTransactionsForCurrentUser === 'function') initTransactionsForCurrentUser(); } catch (e) { /* ignore */ }
      try { if (window.__balanceCtrl && typeof window.__balanceCtrl.start === 'function') window.__balanceCtrl.start(); } catch (e) { /* ignore */ }

      try {
        document.querySelectorAll('.swiper, .swiper-container').forEach(el => {
          const i = el.swiper || el.__swiper;
          if (i && i.autoplay && typeof i.autoplay.start === 'function') {
            try { i.autoplay.start(); } catch (e) { /* ignore */ }
          }
        });
        if (window.Swiper && Array.isArray(window.Swiper.instances)) {
          window.Swiper.instances.forEach(s => { s?.autoplay?.start && s.autoplay.start(); });
        }
      } catch (e) { /* ignore */ }
      ...
      try {
        if (window.Chart && typeof Chart.getChart === 'function') {
          document.querySelectorAll('canvas').forEach(c => {
            try { const chart = Chart.getChart(c); if (chart && typeof chart.update === 'function') chart.update(); } catch (e) { /* ignore */ }
          });
        }
      } catch (e) { /* ignore */ }
    ...
    if ('requestIdleCallback' in window) requestIdleCallback(resume, { timeout: 1000 });
    else setTimeout(resume, 350);
  }

  document.addEventListener('visibilitychange', function () {
    if (document.hidden) pauseHeavyWork(); else resumeHeavyWork();
  }, false);

  // When window regains focus, do light, idle refreshes (prevents a big synchronous reflow)
  (function () {
    let t;
    window.addEventListener('focus', function () {
      ...
      t = setTimeout(function () {
        ...
        if ('requestIdleCallback' in window) {
          requestIdleCallback(() => { try { if (typeof initTransactionsForCurrentUser === 'function') initTransactionsForCurrentUser(); } catch (e) { } }, { timeout: 1000 });
        } else {
          setTimeout(() => { try { if (typeof initTransactionsForCurrentUser === 'function') initTransactionsForCurrentUser(); } catch (e) { } }, 300);
        }
      }, 180);
    }, false);
  })();

  // Micro UX/perf improvements:
  // - hint the browser we want fast taps (reduce 300ms legacy delay on some devices)
  try { document.documentElement.style.touchAction = document.documentElement.style.touchAction || 'manipulation'; } catch (e) { /* ignore */ }

  // quick CSS to massively reduce animations when paused
  const css = `.app-paused * { animation-play-state: paused !important; transition-duration: 0s !important; }`;
  const style = document.createElement('style'); style.appendChild(document.createTextNode(css)); document.head.appendChild(style);

  // expose a tiny helper for future tweaks
  window.GlobalsPerf = {
    pushTabState,
    pauseHeavyWork,
    resumeHeavyWork
