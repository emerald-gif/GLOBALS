...
/* ===========================================================
  THE ULTIMATE FINTECH MANEUVER (OPAY/KUDA STYLE)
  Just paste and forget. No HTML editing required.
  ===========================================================
*/
(function() {
    // 1. CREATE THE STYLES (Modern Fintech Glassmorphism)
    const style = document.createElement('style');
    style.innerHTML = `
        @keyframes master-spin { to { transform: rotate(360deg); } }
        @keyframes master-pulse { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }
        ...
        .loader-content { position: relative; display: flex; align-items: center; justify-content: center; flex-direction: column; }
        
        .loader-ring {
            width: 90px; height: 90px;
            border: 3px solid #f0f0f0;
            border-top: 3px solid #000; /* Clean Black like ChatGPT/Kuda */
            border-radius: 50%;
        ...
        .loader-logo {
            ...
            border-radius: 18px; /* Squircle Fintech style */
            object-fit: cover;
        ...
        .loader-text {
            ...
            letter-spacing: 0.5px; animation: master-pulse 1.5s infinite;
        }
    `;
    document.head.appendChild(style);

    // 2. CREATE THE LOADER HTML AUTOMATICALLY
    const loaderDiv = document.createElement('div');
    loaderDiv.id = 'global-app-loader';
    loaderDiv.innerHTML = `
        <div class="loader-content">
            <div class="loader-ring"></div>
            <img src="GLOBALS.jpg" class="loader-logo" onerror="this.src='https://via.placeholder.com/60'">
            <p class="loader-text">Loading...</p>
        </div>
    `;
    document.body.appendChild(loaderDiv);

    // 3. THE MANEUVER LOGIC (Force it to run on any click)
    function triggerLoader() {
        loaderDiv.style.display = 'flex';
        loaderDiv.style.opacity = '1';

        // Hide it after 1.5 seconds automatically
        setTimeout(() => {
    ...
    // Capture clicks on ALL buttons, tabs, and submits site-wide
    document.addEventListener('click', function(e) {
        const isClickable = e.target.closest('button, .tab-btn, .btn, input[type="submit"], a');
        if (isClickable) {
            triggerLoader();
        }
    }, true); // The "true" makes this fire BEFORE any other script

    // Catch all form submissions (like adding a school)
