...
(() => {
  ...
  /* =========================================
     Portrait Receipt Builder (bank-style) - unchanged style
     ========================================= */
  ...
  /* =========================================
     In-page share modal (no new tabs)
     ========================================= */
  ...
  /* =========================================
     Share-only portrait flow (no downloads, no new tabs)
     ========================================= */
      ...
      try {
        if (navigator.share && navigator.canShare) {
          try {
            ...
            // some platforms throw when checking canShare - ignore and fall through to modal
            console.warn("navigator.canShare threw or returned false:", err);
          }
        }
      } catch (err) {
        console.warn("Web Share with files failed/cancelled:", err);
      }

      // If we reach here: we can't share files via navigator.share on this platform
      // Show in-page modal where user can copy image to clipboard or try a text share fallback
      const modal = createShareModal();
      modal.imgEl.src = canvas.toDataURL("image/png");
        ...
        try {
          if (navigator.clipboard && navigator.clipboard.write) {
            ...
            try {
              ...
              // fallback to try writeText with dataUrl
            }
          }

          // fallback: write dataURL as text (user can paste it somewhere)
          if (navigator.clipboard && navigator.clipboard.writeText) {
            try {
              const dataUrl = canvas.toDataURL("image/png");
              await navigator.clipboard.writeText(dataUrl);
              modal.copyBtn.innerText = "Data URL Copied ✓";
              setTimeout(() => { modal.copyBtn.innerText = prev; modal.copyBtn.disabled = false; }, 1500);
              return;
            } catch (err) {
          ...
          // if we reach here, copying not supported
          alert("Copying the image isn't supported in this browser. Long-press the image on mobile to save/share it.");
        } finally {
          try { modal.copyBtn.disabled = false; } catch {}
        }
      };

      // system share (text fallback) handler
      modal.systemShareBtn.onclick = async () => {
        modal.systemShareBtn.disabled = true;
        const old = modal.systemShareBtn.innerText;
        modal.systemShareBtn.innerText = "Opening…";
        try {
          if (navigator.share) {
            // try at least sharing text; some platforms support this even if files were not allowed
            await navigator.share({
              title: `Globals Receipt (${tx.id})`,
              text: `Transaction ${tx.id} — ${formatAmount(tx.amount)}\nOpen the Globals app or website to view details.`,
            });
          } else {
            alert("System share is not supported in this browser. Use 'Copy Image' or long-press the image to save/share.");
          }
        } catch (err) {
          console.warn("System share failed/cancelled:", err);
  ...
  console.log("transactions-complete.js loaded: share-only portrait receipts ready (no new-tab fallback).");
})();
