...
function showTaskDetails(jobId, jobData) {
  ...
  // ✅ Add event listener after DOM is added USING CLOUDINARY FOR NOW
  ...
  const cloudName = "dyquovrg3"; // Replace with your Cloudinary cloud name
  const uploadPreset = "globals_tasks_proofs"; // Replace with your upload preset
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (file) {
      ...
      try {
        ...
        const data = await res.json();
        uploadedFiles.push(data.secure_url); // Cloudinary returns secure URL
      } catch (err) {
...
async function submitTask() {
  ...
  try {
    ...
    const cloudName = "dyquovrg3"; // ← Replace with your Cloudinary cloud name
    const uploadPreset = "globals_tasks_proofs";

    const formData = new FormData();
    formData.append("file", screenshotExample);
    formData.append("upload_preset", uploadPreset);
