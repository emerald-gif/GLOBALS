...
// ---- Listener registry: centralize Firestore listeners & cleanup ----
window.__listenerRegistry = (function(){
  const subs = new Set();
  return {
    register(fn){ if (typeof fn === 'function') subs.add(fn); },
    unregister(fn){ if (typeof fn === 'function') subs.delete(fn); },
    clearAll(){ subs.forEach(u=>{ try{ u(); }catch(e){} }); subs.clear(); },
    count(){ return subs.size; }
  };
})();

// helper to auto-register unsubscribe returned from onSnapshot
function __registerUnsub(unsub){
  try{ if (typeof unsub === 'function') window.__listenerRegistry.register(unsub); }catch(e){ console.warn('registerUnsub failed',e); }
  return unsub;
}

// Monkey-patch Firestore onSnapshot to auto-register unsubs for Query and DocumentReference
try {
  if (firebase && firebase.firestore && firebase.firestore.Query && firebase.firestore.Query.prototype && firebase.firestore.DocumentReference && firebase.firestore.DocumentReference.prototype) {
    (function(){
      const qProto = firebase.firestore.Query.prototype;
      const docProto = firebase.firestore.DocumentReference.prototype;
      const origQ = qProto.onSnapshot;
      const origDoc = docProto.onSnapshot;
      if (origQ && !origQ.__wrapped) {
        qProto.onSnapshot = function(...args){
          const unsub = origQ.apply(this,args);
          return __registerUnsub(unsub);
        };
        qProto.onSnapshot.__wrapped = true;
      }
      if (origDoc && !origDoc.__wrapped) {
        docProto.onSnapshot = function(...args){
          const unsub = origDoc.apply(this,args);
          return __registerUnsub(unsub);
        };
        docProto.onSnapshot.__wrapped = true;
      }
    })();
  }
} catch(e){ console.warn('failed to monkey-patch onSnapshot', e); }

// Current section cleanup holder
...
firebase.auth().onAuthStateChanged((user) => {
  ...
  if (_switchTab) {
    window.switchTab = async function (sectionId) {
      if (await isPremiumAllowed(sectionId)) {
        // Cleanup previous section listeners/timers
        try {
          if (typeof window.__currentCleanup === 'function') {
            try { window.__currentCleanup(); } catch(e){ console.warn('section cleanup failed', e); }
            window.__currentCleanup = null;
          }
        } catch(e){ console.warn('cleanup check failed', e); }
        // Clear any registered Firestore listeners as a safety net
        try { if (window.__listenerRegistry && typeof window.__listenerRegistry.clearAll === 'function') window.__listenerRegistry.clearAll(); } catch(e){ console.warn('clearAll failed', e); }
...
// Prevent lag when tab is hidden or Chrome throttles JS timers
let _listenersPaused = false;
let _resumeTimeout = null;

function pauseAllRealtimeListeners() {
  try {
    if (typeof stopTransactionsListener === 'function') stopTransactionsListener();
  } catch (e) {}
  try { if (typeof _autoCycleInterval !== 'undefined' && _autoCycleInterval) clearInterval(_autoCycleInterval); } catch (e) {}
  _listenersPaused = true;
  console.log('[Performance] 🔇 Paused realtime listeners due to tab hidden');
}

function resumeAllRealtimeListeners() {
  if (_resumeTimeout) clearTimeout(_resumeTimeout);
  _resumeTimeout = setTimeout(() => {
    ...
    } catch (e) { console.error('[Performance] resumeAllRealtimeListeners error:', e); }
    _listenersPaused = false;
    console.log('[Performance] 🔊 Resumed realtime listeners after tab became visible');
  }, 400); // slight debounce to avoid lag burst
}

document.addEventListener('visibilitychange', () => {
  if (document.hidden && !_listenersPaused) {
    pauseAllRealtimeListeners();
  } else if (!document.hidden && _listenersPaused) {
    resumeAllRealtimeListeners();
  }
});
