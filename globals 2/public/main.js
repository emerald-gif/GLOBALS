...
function initTaskSection() {
  ...
  async function showTaskDetails(jobId, jobData) {
    ...
    fullScreen.innerHTML = `
      <div class="max-w-2xl mx-auto space-y-6">
        ...
        <img src="${escapeHtml(jobData.screenshotURL || 'https://via.placeholder.com/400')}"
          alt="Task Preview"
    ...
    // If user already submitted -> show submission (one-time check)
    try {
    ...
    // attach submit handler if not submitted
  ...
  // ---------- Task card renderer (one-time, used by fetchTasksOnce) ----------
  ...
  // ---------- One-time task fetch (no live listeners) ----------
...
// AFF2 - Final Robust Affiliate Module (no onSnapshot, UI unchanged)
(function(){
  'use strict';

  if (window.AFF2_FINAL_INSTANCE) { console.warn('[AFF2] already initialized'); return; }
  window.AFF2_FINAL_INSTANCE = true;
  ...
  function makeJobCard(job, occupancy, approved) {
    ...
    wrap.innerHTML = `
      ...
      <div class="mt-3">
        ...
        <div class="mt-3">
          <button class="aff2-btn-view w-full py-2 bg-blue-600 text-white rounded-xl" data-id="${safeText(job.id)}">View Task</button>
        </div>
      </div>
    `;
    return wrap;
  ...
  async function loadAndRenderJobs() {
    ...
    if (!grid) return console.warn('[AFF2] jobs container missing');
    try {
      ...
      // fetch occupancy and approved per job in parallel
      const counts = await Promise.all(jobs.map(async job => {
        ...
        try {
          const occSnap = await db.collection('affiliate_submissions')
            ...
            .get();
          occupancy = occSnap.size;
        } catch(e) {
          const all = await db.collection('affiliate_submissions').where('jobId','==', jobId).get();
          occupancy = all.docs.filter(d => {
      ...
      for (const job of jobs) {
        ...
        if (numWorkers > 0 && c.approved >= numWorkers) {
          // fully approved => job leaves list
          continue;
        }
  ...
  async function openJobDetail(jobId) {
    ...
    try {
      ...
      try {
        ...
        const all = await db.collection('affiliate_submissions').where('jobId','==', job.id).get();
        occupancy = all.docs.filter(d => {
          const s = (d.data().status||'').toLowerCase();
          return s === 'on review' || s === 'approved';
        }).length;
      ...
      body.innerHTML = `
        ...
        <div>
          ...
          <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
            <div id="aff2_detailProgressBar" class="h-2 bg-blue-500 rounded-full" style="width:${Math.min(100, Math.round((state.currentOccupancy / Math.max(1, Number(job.numWorkers||1))) * 100))}%"></div>
          </div>
        ...
        <div id="aff2_submitArea" class="mt-6 border-t pt-4">
          ...
          <p id="aff2_submitHelp" class="text-xs text-gray-400 mt-2">Submissions are reviewed by admin.</p>
        </div>
      `;
      ...
      if (numWorkers > 0 && state.currentApproved >= numWorkers) {
        ...
        if (area) area.innerHTML = `<div style="padding:12px;background:#f8fafc;border-radius:12px;text-align:center"><strong>All slots fully approved. This job is closed.</strong></div>`;
        return;
      }

      if (numWorkers > 0 && state.currentOccupancy >= numWorkers) {
        const area = el('aff2_submitArea');
        if (area) area.innerHTML = `<div style="padding:12px;background:#fffbeb;border-radius:12px;text-align:center"><strong>No open submission slots right now. Please check back later.</strong></div>`;
      ...
      const uid = user.uid;
      const existingSnap = await db.collection('affiliate_submissions').where('jobId','==', job.id).where('userId','==', uid).get();
  ...
  async function showUserSubmissionIfExists(jobId) {
    ...
    try {
      ...
      const snap = await db.collection('affiliate_submissions').where('jobId','==', jobId).where('userId','==', uid).get();
      if (!snap.empty) {
  ...
  function replaceSubmitAreaWithSubmitted(sub) {
    ...
    const proofs = urls.map(u => `<img src="${safeText(u)}" style="width:100%;max-height:220px;object-fit:contain;border-radius:8px;margin-bottom:8px">`).join('');
  ...
  function updateDetailProgressUI() {
    ...
    const total = Math.max(1, Number(state.currentNumWorkers || 1));
    ...
    if (text) text.textContent = `${occ}/${Number(state.currentNumWorkers||0)} · Approved: ${approved}`;
  }
  ...
  function attachSubmitHandler(job) {
    ...
    submitBtn.onclick = async function() {
      ...
      try {
        const dup = await db.collection('affiliate_submissions').where('jobId','==', job.id).where('userId','==', uid).get();
        if (!dup.empty) {
        ...
        // increment occupancy locally and update UI
        state.currentOccupancy = (state.currentOccupancy || 0) + 1;
        updateDetailProgressUI();
  ...
  async function loadAndRenderFinishedList() {
    ...
    try {
      ...
      console.error('[AFF2] loadAndRenderFinishedList', err);
      listEl.innerHTML = '<p class="text-gray-500 p-6">No finished tasks yet.</p>';
    }
  }
  ...
  function onDocumentClick(e) {
    const t = e.target;
    const viewJob = t.closest && t.closest('.aff2-btn-view');
    if (viewJob) { const id = viewJob.dataset.id; if (id) openJobDetail(id); return; }

    const viewSub = t.closest && t.closest('.aff2-view-submission-btn');
    if (viewSub) { const id = viewSub.dataset.id; if (id) openSubmissionModal(id); return; }
  ...
  function destroy() {
    ...
    try { delete window.AFF2_FINAL_INSTANCE; } catch(_) { window.AFF2_FINAL_INSTANCE = null; }
  }

  window.AFF2 = { init, destroy, loadAndRenderJobs, loadAndRenderFinished: loadAndRenderFinishedList, openJobDetail };
