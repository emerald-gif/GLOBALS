...
(() => {
  ...
  async function eduSelectProvider(id){
    ...
    try{
      ...
      const plans=data.plans||[];
      if(!resp.ok||plans.length===0){
        const errMsg=data.error||'No products found — please whitelist this service on your VTPass dashboard.';
        if(plansWrap)plansWrap.innerHTML=
          `<p style="font-size:0.78rem;color:#ef4444;padding:8px 0;font-family:'DM Sans',sans-serif;">${errMsg}</p>`+
      ...
      if(plansWrap)plansWrap.innerHTML=
        '<p style="font-size:0.78rem;color:#ef4444;padding:8px 0;font-family:\'DM Sans\',sans-serif;">Could not load products. Check your connection.</p>'+
        `<button onclick="eduSelectProvider('${id}')" style="margin-top:6px;padding:8px 16px;border-radius:10px;border:1.5px solid #1a6fff;background:#fff;color:#1a6fff;font-family:'Sora',sans-serif;font-size:0.8rem;font-weight:700;cursor:pointer;">↻ Retry</button>`;
    }
  }
