...
/* --------------------- State --------------------- */
let adminSwiperInstance = null;
let dashboardListenersInitialized = false;

/* --------------------- Render functions --------------------- */
function renderAdminSwiper(jobs) {
  const slidesContainer = document.getElementById("adminJobsSlides");
  const frag = document.createDocumentFragment();

  if (!jobs.length) {
    const slide = document.createElement("div");
    slide.className = "swiper-slide px-2";
    slide.innerHTML = `<div class="rounded-2xl p-6 bg-white card-soft text-center"><p class="text-gray-500">No featured jobs</p></div>`;
    frag.appendChild(slide);
  } else {
    jobs.forEach(job => {
      const wrapper = document.createElement("div");
      wrapper.className = "swiper-slide px-2";
      const img = job.campaignLogoURL || job.image || "/assets/default-banner.jpg";
      wrapper.innerHTML = `
        <div class="rounded-2xl overflow-hidden bg-white card-soft">
          <div class="relative">
            <img src="${escapeHtml(img)}" alt="${escapeHtml(job.title)}" class="w-full h-56 object-cover rounded-t-2xl">
            <div class="absolute -bottom-6 left-6">
              <img src="${escapeHtml(job.campaignLogoURL || img)}" class="w-14 h-14 rounded-full border-4 border-white object-cover shadow" />
            </div>
          </div>
          <div class="p-4 pt-8">
            <h3 class="text-lg font-bold">${escapeHtml(job.title)}</h3>
            <p class="text-sm text-gray-500 mt-1">${formatNaira(job.workerPay)} · ${job.numWorkers || 0} workers</p>
            <button class="mt-4 w-full py-3 rounded-xl bg-blue-600 text-white font-semibold btn-big view-admin-job" data-id="${job.id}" data-type="admin">View</button>
          </div>
        </div>`;
  ...
  requestAnimationFrame(() => {
    ...
    if (adminSwiperInstance) {
      adminSwiperInstance.update();
    } else {
      adminSwiperInstance = new Swiper("#adminJobsSwiper", {
        slidesPerView: 1.05,
        spaceBetween: 12,
        centeredSlides: true,
        loop: (jobs.length > 1),
        pagination: { el: ".my-admin-swiper .swiper-pagination", clickable: true },
        observer: true,
        observeParents: true,
        preloadImages: true,
...
function renderAffiliateGrid(jobs) {
  const grid = document.getElementById("affiliateGrid");
  const frag = document.createDocumentFragment();

  if (!jobs.length) {
    const empty = document.createElement("div");
    empty.className = "col-span-2 p-6 bg-white rounded-xl card-soft";
    empty.textContent = "No affiliate tasks currently.";
    frag.appendChild(empty);
  } else {
    jobs.forEach(job => {
      const accepted = job.filledWorkers || 0;
      ...
      const card = document.createElement("div");
      card.className = "bg-white rounded-2xl card-soft p-3";
      card.innerHTML = `
        <div class="overflow-hidden rounded-xl">
          <img src="${escapeHtml(img)}" alt="${escapeHtml(job.title)}" class="w-full h-36 object-cover rounded-xl">
        </div>
        <div class="mt-3">
          <h4 class="font-semibold text-md">${escapeHtml(job.title)}</h4>
          ...
          <button class="mt-4 w-full py-3 rounded-xl bg-blue-600 text-white font-semibold view-affiliate-task" data-id="${job.id}" data-type="affiliate">View Task</button>
        </div>`;
...
/* --------------------- Lazy initialize listeners when section visible --------------------- */
function initJobsUIOnDashboardOnce() {
  if (dashboardListenersInitialized) return;
  dashboardListenersInitialized = true;

  // adminJobs swiper (featured admin jobs)
  db.collection("adminJobs").where("status", "==", "active")
    .onSnapshot(snap => {
      const jobs = [];
      snap.forEach(doc => { const d = doc.data(); d.id = doc.id; jobs.push(d); });
      jobs.sort((a,b) => (b.postedAt?.seconds || 0) - (a.postedAt?.seconds || 0));
      renderAdminSwiper(jobs);
    }, err => console.error("adminJobs listener error:", err));

  // affiliate jobs (approved)
  db.collection("affiliateJobs").where("status", "==", "approved")
    .onSnapshot(snap => {
      const jobs = [];
      snap.forEach(doc => { const d = doc.data(); d.id = doc.id; jobs.push(d); });
      jobs.sort((a,b) => (b.postedAt?.seconds || 0) - (a.postedAt?.seconds || 0));
      renderAffiliateGrid(jobs);
    }, err => console.error("affiliateJobs listener error:", err));
}

/* Observe visibility of #affiliateSection to lazy-init */
(function observeAffiliateSection() {
  const section = document.getElementById("affiliateSection");
  if (!section) { initJobsUIOnDashboardOnce(); return; } // fallback
  const observer = new IntersectionObserver((entries, obs) => {
    entries.forEach(en => {
      if (en.isIntersecting) {
        initJobsUIOnDashboardOnce();
        obs.disconnect();
      }
    });
...
/* --------------------- Modal handlers + submit proof --------------------- */
let currentModalJob = null;

function openJobModal(job) {
  currentModalJob = job;
  document.getElementById("modalBannerImg").src = job.campaignLogoURL || job.image || "/assets/default-banner.jpg";
  document.getElementById("modalSmallAvatar").src = job.campaignLogoURL || job.image || "/assets/default-thumb.jpg";
  document.getElementById("modalJobTitle").textContent = job.title;
  document.getElementById("modalMeta").textContent = `${formatNaira(job.workerPay)} · ${job.numWorkers || 0} workers`;
  document.getElementById("modalInstructions").textContent = job.instructions || "";
  const accepted = job.filledWorkers || 0;
  const total = job.numWorkers || 0;
  const percent = total > 0 ? Math.min(100, Math.round((accepted / total) * 100)) : 0;
  document.getElementById("modalProgressBar").style.width = percent + "%";
  document.getElementById("modalProgressText").textContent = `${accepted}/${total} · ${percent}%`;
  document.getElementById("modalProofReq").textContent = `Proofs required: ${job.proofFileCount || 1}`;
...
/* View button delegated - open modal */
document.addEventListener("click", async (e) => {
  const adminBtn = e.target.closest(".view-admin-job");
  if (adminBtn) {
    const id = adminBtn.dataset.id;
    // fetch doc and show same modal but treat as admin-job
    const doc = await db.collection("adminJobs").doc(id).get();
    if (!doc.exists) return alert("Job not found");
    const job = doc.data(); job.id = doc.id;
    openJobModal(job);
    return;
  }
  const affBtn = e.target.closest(".view-affiliate-task");
  if (affBtn) {
    const id = affBtn.dataset.id;
    const doc = await db.collection("affiliateJobs").doc(id).get();
    if (!doc.exists) return alert("Job not found");
    const job = doc.data(); job.id = doc.id;
    openJobModal(job);
    return;
...
document.getElementById("submitProofBtn").addEventListener("click", async () => {
  if (!auth.currentUser) return alert("You must be logged in to submit proof.");
  if (!currentModalJob) return alert("No job selected.");

  const filesInput = document.getElementById("submissionFiles");
  const files = filesInput.files || [];
  if (!files.length) return alert("Attach at least one proof file.");

  const required = currentModalJob.proofFileCount || 1;
  if (files.length < required) {
    if (!confirm(`This job requires ${required} proofs. Continue with ${files.length}?`)) return;
  }
  ...
  try {
    // upload all files (use your uploadToCloudinary helper)
    const uploads = [];
    for (let i = 0; i < files.length; i++) {
      uploads.push(uploadToCloudinary(files[i])); // must return URL string
    }
    const urls = await Promise.all(uploads);

    // create submission doc
    const submission = {
      jobId: currentModalJob.id,
      userId: auth.currentUser.uid,
      userName: auth.currentUser.displayName || auth.currentUser.email || "",
      postedAt: firebase.firestore.FieldValue.serverTimestamp(),
      proofURLs: urls,
      note: document.getElementById("submissionNote").value || "",
      status: "pending" // admin will change to "approved" when accepted
    };

    await db.collection("affiliate_submissions").add(submission);

    alert("✅ Proof submitted! It will be reviewed shortly.");
    closeJobModal();
  } catch (err) {
