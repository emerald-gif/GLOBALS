  ...
  try {
    ...
    function _renderNotifSnap(snapshot, lastReadAt, clearedAt, joinedAt) {
      ...
      snapshot.forEach(doc => {
        ...
        card.style.cssText = 'background:#fff;border-radius:16px;padding:16px 18px;box-shadow:0 2px 12px rgba(8,20,40,0.07);border:1px solid #e4e8f0;display:flex;flex-direction:column;gap:0;cursor:pointer;transition:box-shadow 0.18s,transform 0.18s;';

        const topRow = document.createElement('div');
        topRow.style.cssText = 'display:flex;align-items:flex-start;gap:12px;';

        const iconWrap = document.createElement('div');
        iconWrap.style.cssText = 'width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#1a6fff22,#1a6fff11);display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:2px;';
        iconWrap.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#1a6fff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg>';
      ...
      if (notifDotEl)  notifDotEl.classList.toggle('hidden', unreadCount === 0);
      if (notifPopupEl && notifMsgEl) {
        if (unreadCount > 0) {
          notifMsgEl.textContent = `You have ${unreadCount} new notification${unreadCount > 1 ? 's' : ''}`;
          notifPopupEl.classList.remove('hidden');
        } else { notifPopupEl.classList.add('hidden'); }
      }
      ...
      if (notifListEl.children.length === 0) {
        notifListEl.innerHTML = `<p class="text-gray-400 text-center py-8">No notifications yet.</p>`;
      }
    }
...
async function submitWithdrawal() {
  ...
  if (!accountName || accountName.startsWith('❌') || accountName.startsWith('Error')) {
    alert('⚠️ Please verify your account number first before withdrawing.');
    return;
  }
...
(function() {
  ...
  const premModalHtml = `
  <div id="premiumConfirmModal" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(0,0,0,0.5);align-items:center;justify-content:center;padding:16px;">
    <div style="background:#fff;border-radius:24px;padding:28px 20px;max-width:360px;width:100%;text-align:center;">
      <div style="font-size:3rem;margin-bottom:12px;">👑</div>
      <h3 style="font-family:'Sora',sans-serif;font-size:1.2rem;font-weight:800;color:#0d1b2e;margin:0 0 8px;">Confirm Premium Upgrade</h3>
      <p style="font-family:'DM Sans',sans-serif;font-size:0.88rem;color:#6b7280;margin:0 0 20px;line-height:1.5;">You're about to unlock Premium access. Enjoy exclusive rewards, daily bonuses, and more!</p>
      <div style="background:#fffbeb;border-radius:14px;padding:14px;margin-bottom:20px;text-align:left;">
        <p style="font-family:'DM Sans',sans-serif;font-size:0.78rem;color:#92400e;margin:0 0 4px;"><strong>What you get:</strong></p>
        <p style="font-family:'DM Sans',sans-serif;font-size:0.78rem;color:#78350f;margin:0;line-height:1.6;">✅ Daily check-in bonuses<br>✅ Exclusive premium tasks<br>✅ Priority withdrawals<br>✅ Premium badge on profile</p>
      </div>
  ...
  window._doPremiumUpgrade = async function() {
    ...
    // Delegate to existing premium handler if present
    try {
      if (typeof window.handlePremiumUpgrade === 'function') {
        await window.handlePremiumUpgrade();
      } else {
