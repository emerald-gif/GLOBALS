...
(() => {
  ...
  function createCardElement(tx) {
    ...
    wrapper.innerHTML = `
      <div class="flex items-center justify-between">
        <div>
          <p class="text-sm font-semibold text-gray-900">${tx.type || "Transaction"}</p>
        ...
        <div class="text-right">
          ...
          <span class="inline-block mt-1 px-2 py-0.5 text-xs rounded-full ${amountClass} bg-opacity-10">${statusDisplay}</span>
        </div>
      </div>
    `;
  ...
  window.openTransactionDetails = function openTransactionDetails(id) {
    try {
      ...
      let extraRows = '';
      if (type === 'airtime') {
        extraRows = row('Network', tx.network) + row('Phone Number', tx.phone);
      } else if (type === 'data') {
        extraRows = row('Network', tx.network) + row('Phone Number', tx.phone) + row('Data Plan', tx.planLabel);
      } else if (type === 'electricity') {
        extraRows = row('Distributor', tx.distro||tx.serviceID) + row('Meter Number', tx.meterNumber, true) + row('Meter Type', tx.meterType) + row('Customer Name', tx.customerName) + (tx.token ? row('⚡ Token', tx.token, true) : '');
      } else if (type === 'tv') {
        extraRows = row('Provider', tx.provider||tx.serviceID) + row('Smartcard / IUC', tx.smartcard, true) + row('Plan', tx.planLabel) + row('Customer', tx.customerName);
      } else if (type === 'betting') {
        extraRows = row('Platform', tx.platform||tx.serviceID) + row('Account / User ID', tx.customerID, true);
      } else if (type === 'education') {
        extraRows = row('Exam Type', tx.examType||tx.serviceID) + row('Product', tx.planLabel) + row('Phone', tx.phone) + row('Quantity', tx.quantity) + (tx.eduPin ? row('🎓 Exam PIN', tx.eduPin, true) : '');
      } else if (type === 'withdraw') {
        extraRows = row('Bank', tx.bankName) + row('Account Name', tx.account_name) + row('Account Number', tx.accNum, true);
      }
...
(() => {
  ...
  async function elecVerifyMeter() {
    ...
    const btn = document.querySelector('#elec-body button[onclick="elecVerifyMeter()"]');
    ...
    try {
      ...
      const data = await resp.json();
      if (data.code === '00' || data.content?.Customer_Name) {
        const name = data.content?.Customer_Name || data.content?.name || 'Verified';
        const address = data.content?.Address || data.content?.address || '';
        ...
        showErr('elec-error', data.response_description || 'Could not verify meter. Check number and distributor.');
      }
    } catch (e) { showErr('elec-error', 'Verification failed. Check connection.'); }
    finally { if(btn){btn.textContent='Verify Meter';btn.disabled=false;} }
  }
  ...
  async function elecPay() {
    ...
    try {
      ...
      if (data.requestId && typeof db!=='undefined') {
        const unsub = db.collection('Transaction').doc(data.requestId).onSnapshot(snap => {
          ...
          if (d.status === 'success') {
            if (d.token) {
              document.getElementById('elec-token-display').textContent = d.token;
    ...
    } catch(e) { showErr('ec-error','Network error. Try again.'); window.btnLoad(btn, false, 'Pay Now'); }
  }
  ...
  function electricityReset() {
    ...
    document.querySelectorAll('#elec-provider-grid .svc-provider-btn').forEach(b=>b.classList.remove('selected'));
    window._renderRecentChips('electricity','elec-meter','elec-recent');
    _show('elec-screen');
  }
})();
...
(() => {
  ...
  function tvOpen() {
    ...
    window._setupAutoVerify('tv-smartcard', 10, () => {
      if (tvProvider) tvVerifySmartcard();
    });
  }
  ...
  async function tvVerifySmartcard() {
    ...
    const btn = document.querySelector('#tv-screen button[onclick="tvVerifySmartcard()"]');
    if (btn) { btn.textContent='Verifying…'; btn.disabled=true; }
    ...
    try {
      ...
      const data = await resp.json();
      if (data.code==='00' || data.content?.Customer_Name || data.content?.name) {
        const name = data.content?.Customer_Name || data.content?.name || 'Verified';
        const plan = data.content?.Current_Bouquet || data.content?.current_bouquet || '';
        tvVerifiedName = name;
        ...
        showErr('tv-error', data.response_description||'Could not verify smartcard.');
      }
    } catch(e) { showErr('tv-error','Verification failed. Check connection.'); }
    finally { if(btn){btn.textContent='Verify Smartcard';btn.disabled=false;} }
  }
  ...
  function tvReset() {
    ...
    tvVerifiedName=''; tvProvider=''; tvSelectedPlan=null;
    ...
    document.querySelectorAll('#tv-provider-grid .svc-provider-btn').forEach(b=>b.classList.remove('selected'));
    window._renderRecentChips('tv','tv-smartcard','tv-recent');
    _show('tv-screen');
  }
})();
...
(() => {
  ...
  async function betPay(){
    ...
    }catch(e){showErr('bet-error-confirm','Network error. Try again.');_betPaying=false;window.btnLoad(btn,false,'Fund Now');}
  }
  ...
  function bettingReset(){
    ...
    document.querySelectorAll('#bet-provider-grid .svc-provider-btn').forEach(b=>b.classList.remove('selected'));
    window._renderRecentChips('betting','bet-customerid','bet-recent');
    _show('bet-screen');
  }
})();
...
(() => {
  ...
  async function eduPay(){
    ...
    try{
      ...
      if(data.requestId&&typeof db!=='undefined'){
        const unsub=db.collection('Transaction').doc(data.requestId).onSnapshot(snap=>{
          ...
          if(d.status==='success'){
            if(d.eduPin){
              document.getElementById('edu-pin-display').textContent=d.eduPin;
    ...
    }catch(e){showErr('edu-error-confirm','Network error. Try again.');window.btnLoad(btn,false,'Buy Now');}
  }

  function educationReset(){
    document.getElementById('edu-phone').value='';
    const pin=document.getElementById('edu-pin');if(pin)pin.value='';
    eduProvider=''; eduSelectedPlan=null;
