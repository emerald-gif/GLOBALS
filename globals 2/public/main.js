...
/* === Assistant Inline Fixes === */
(function(){
  function qs(s,r){return (r||document).querySelector(s);}
  function qsa(s,r){return Array.from((r||document).querySelectorAll(s));}

  // Back button logic
  const backBar = qs('#backArrowBar');
  if(backBar){
    const backBtn = qs('#backButton', backBar);
    if(backBtn){
      backBtn.onclick = ()=>{ if(typeof activateTab==='function'){activateTab('dashboard');} };
    }
  }

  // Enhance activateTab to toggle back bar
  if(typeof activateTab==='function'){
    const orig=activateTab;
    window.activateTab=function(id){
      orig(id);
      if(backBar){
        if(id && id!=='dashboard'){ backBar.classList.remove('hidden'); }
        else{ backBar.classList.add('hidden'); }
      }
    };
  }

  // Affiliate section ensure visible
  ['#mainAffiliateScreen','#affiliateSection'].forEach(sel=>{
    const el=qs(sel); if(el){ el.classList.remove('hidden'); el.style.display=''; }
  });

  // Hide eBook/NFT/Spin UI
  qsa('*').forEach(el=>{
    const t=(el.innerText||'').toLowerCase();
    if(t.includes('nft')||t.includes('spin')||t.includes('ebook')){ el.style.display='none'; }
  });

  // Notifications filter
  window.toggleNotifPopup=function(){
    const p=qs('#notifPopup'); if(!p) return;
    if(p.classList.contains('hidden')){
      p.classList.remove('hidden'); p.style.display='';
      localStorage.setItem('globals_last_notif_seen', Date.now().toString());
    }else{ p.classList.add('hidden'); p.style.display='none'; }
  };
  window.closeNotifPopup=function(){const p=qs('#notifPopup'); if(p){p.classList.add('hidden'); p.style.display='none';}};
  const lastSeen=Number(localStorage.getItem('globals_last_notif_seen')||'0');
  qsa('[data-timestamp]').forEach(it=>{ const ts=Number(it.dataset.timestamp||'0'); if(ts<=lastSeen) it.style.display='none'; });

  // Logout fix
  window.logoutUser=function(){
    ['auth_token','user_id','user_session','globals_last_notif_seen'].forEach(k=>localStorage.removeItem(k));
    location.href='login.html';
  };

  // Services drawer
  window.openServicesDrawer=function(){const d=qs('#servicesDrawer'); if(d){d.classList.remove('hidden'); d.style.display='block'; document.documentElement.style.overflow='hidden';}};
  window.closeServicesDrawer=function(){const d=qs('#servicesDrawer'); if(d){d.classList.add('hidden'); d.style.display='none'; document.documentElement.style.overflow='';}};

  // Airtime/Data continue button
  ['#airtime-continue','#data-pay-btn'].forEach(sel=>{const el=qs(sel); if(el){el.style.position='relative'; el.style.zIndex='50';}});
