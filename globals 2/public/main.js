...
(() => {
  ...
  // Plans fetched live from VTPass via server (replaces hardcoded DATA_PLANS)
  const _plansCache = {};

  async function _fetchPlans(networkCode) {
    if (_plansCache[networkCode]) return _plansCache[networkCode];
    try {
      const resp = await fetch('/api/vtu/plans/' + networkCode);
      if (!resp.ok) throw new Error('Server error');
      const data = await resp.json();
      if (data.error) throw new Error(data.error);
      const plans = (data.plans || []).map(p => ({
        id:            p.variation_code,
        label:         p.name + ' — ₦' + Number(p.variation_amount).toLocaleString(),
  ...
  async function dataSelectNetwork(code){
    ...
    planWrap.innerHTML='<p style="font-size:0.8rem;color:#9ca3af;text-align:center;padding:12px 0">Loading plans…</p>';

    const plans = await _fetchPlans(code);
    planWrap.innerHTML='';

    if(!plans.length){
      planWrap.innerHTML='<p style="font-size:0.8rem;color:#ef4444;text-align:center;padding:12px 0">Could not load plans. Check connection.</p>';
      return;
    }
    plans.forEach(plan=>{
  ...
  async function dataGoToConfirm(){
    ...
    try{
      ...
      document.getElementById('confirm-data-plan').innerText=plan.planLabel||plan.label;
  ...
  function dataReset(){
    ...
    if(msg) msg.textContent="⏳ Activating your data bundle…"; // reset for next use
