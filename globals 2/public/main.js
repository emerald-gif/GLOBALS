  ...
  function makeJobCard(job, occupancy = 0, approved = 0) {
    ...
    wrap.innerHTML = `
      ...
      <div class="mt-3">
        ...
        <div class="text-sm text-gray-500 mt-1">${formatNaira(job.workerPay)} · ${numWorkers} workers</div>
        <div class="text-sm text-gray-500 mt-1">${occupancy}/${numWorkers} · Approved: ${approved}</div>
...
/*
  Full My Jobs Module Script
  - Supports: tasks & affiliateJobs combined
  - Submissions: task_submissions & affiliate_submissions
  - Money ops use transactions on `users.balance`
*/
...
const ON_REVIEW_STATES = ["on review","on_review","pending","review"];

/* ----- State ----- */
let _currentViewingJob = null; // { id, type, jobDoc }
let _currentSubTab = 'onReview';

/* ----- Init: load both task jobs and affiliate jobs posted by current user ----- */
async function initMyJobsSection() {
  const user = firebase.auth().currentUser;
  if (!user) {
    document.getElementById("jobList").innerHTML = `<div class="text-center text-gray-500">Please log in.</div>`;
    return;
  }

  const jobList = document.getElementById("jobList");
  jobList.innerHTML = `<div class="text-center text-gray-500">Loading your jobs...</div>`;
  ...
  try {
    ...
    affiliateSnap.forEach(doc => jobs.push({ id: doc.id, type: 'affiliate', ...doc.data() }));

    // sort by postedAt (descending)
    jobs.sort((a,b) => {
      const ta = (a.postedAt && a.postedAt.toMillis) ? a.postedAt.toMillis() : 0;
      const tb = (b.postedAt && b.postedAt.toMillis) ? b.postedAt.toMillis() : 0;
      return tb - ta;
    });

    if (!jobs.length) {
      jobList.innerHTML = `<div class="text-center text-gray-400 mt-6">You haven't posted any jobs yet.</div>`;
      return;
    }

    // render
    jobList.innerHTML = jobs.map(j => renderJobCard(j)).join('');
    // attach listeners (cards use onclick with function above)
  } catch (err) {
    console.error("initMyJobsSection error:", err);
    jobList.innerHTML = `<div class="text-center text-red-500">Failed to load jobs.</div>`;
  }

  // ensure lucide icons exist (if lucide loaded)
  if (window.lucide && lucide.createIcons) lucide.createIcons();
}

/* ----- Job card HTML ----- */
function renderJobCard(job) {
  const s = (job.status || "on review").toString();
  const statusLabel = escapeHtml(s);
  const posted = job.postedAt && job.postedAt.toDate ? job.postedAt.toDate().toLocaleString() : (job.postedAt || "—");
  const total = job.total || job.amount || 0;
  const title = escapeHtml(job.title || "Untitled Job");
  const category = escapeHtml(job.category || job.campaign || "Uncategorized");
  const typeLabel = job.type === 'affiliate' ? 'Affiliate' : 'Task';

  // show completed count if available (you may have stored it earlier); optional
  const workers = job.numWorkers || job.targetWorkers || "—";

  return `
    <div class="job-card" style="display:block" onclick="openJobDetails('${job.id}','${job.type}')">
      <div class="job-title">${title} <span style="font-weight:600; font-size:12px; color:#64748b; margin-left:8px">· ${typeLabel}</span></div>
      <div class="job-meta">
        <div class="pill">${category}</div>
        <div class="pill">Status: ${statusLabel}</div>
        <div class="pill">₦${total}</div>
        <div class="pill">Workers: ${workers}</div>
        <div style="margin-left:auto; color:#94a3b8; font-size:12px">Posted: ${posted}</div>
      </div>
    </div>`;
}
...
async function openJobDetails(jobId, jobType) {
  ...
  try {
    ...
    if (!doc.exists) {
      content.innerHTML = `<div class="text-center text-red-500">Job not found.</div>`;
      return;
    }

    const job = { id: doc.id, ...doc.data(), type: jobType };
    _currentViewingJob = { id: jobId, type: jobType, jobDoc: job };

    // get approved count quickly
    const subCollection = jobType === 'task' ? 'task_submissions' : 'affiliate_submissions';
    let approvedCount = 0;
    try {
      const approvedSnap = await db.collection(subCollection).where(jobType === 'task' ? 'taskId' : 'jobId', '==', jobId).where('status','==','approved').get();
      approvedCount = approvedSnap.size || 0;
    } catch(e){ /* ignore */ }

    const canDelete = ON_REVIEW_STATES.includes((job.status||'').toString().toLowerCase());
    const posted = job.postedAt && job.postedAt.toDate ? job.postedAt.toDate().toLocaleString() : (job.postedAt || '—');

    const jobHTML = `
      ${job.campaignLogoURL || job.screenshotURL ? `<img src="${escapeHtml(job.campaignLogoURL || job.screenshotURL)}" class="w-full h-44 object-cover rounded-xl" />` : ''}
      <h3 class="text-xl font-bold">${escapeHtml(job.title || 'Untitled Job')}</h3>
      <p class="text-gray-700">${escapeHtml(job.description || job.note || '')}</p>

      <div class="flex flex-wrap gap-3 mt-3">
        <div class="pill">₦${job.total || job.amount || 0}</div>
        <div class="pill">${escapeHtml(job.status || '—')}</div>
        <div class="pill">Completed: ${approvedCount}</div>
        <div class="pill">Posted: ${posted}</div>
      </div>

      <div class="mt-4 flex gap-3">
        <button class="job-action-btn primary" onclick="viewSubmissions('${job.id}','${job.type}')">
          <i data-lucide="eye" class="w-4 h-4"></i> View Submissions
        </button>

        ${canDelete ? `<button class="job-action-btn secondary" onclick="deleteJob('${job.id}','${job.type}')">
          <i data-lucide="trash" class="w-4 h-4"></i> Delete Job
        </button>` : ''}
      </div>
    `;
    ...
    document.getElementById('jobDetailsContent').innerHTML = `<div class="text-center text-red-500">Failed to load job.</div>`;
  }
}

/* ----- Delete Job (refund to poster) ----- */
async function deleteJob(jobId, jobType) {
  if (!confirm("Delete this job? This action will refund the job poster if job is still on review.")) return;
  try {
    const collection = jobType === 'task' ? 'tasks' : 'affiliateJobs';
    ...
    await db.runTransaction(async tx => {
      ...
      if (!ON_REVIEW_STATES.includes(status)) throw new Error("Job is not on review and cannot be deleted.");

      // determine poster uid (common patterns)
      const posterUid = (job.postedBy && job.postedBy.uid) || job.postedByUid || job.ownerUid || job.ownerId;
      if (!posterUid) throw new Error("Poster UID missing on job.");

      const refundAmount = Number(job.total || job.amount || 0) || 0;
      ...
      tx.update(userRef, { balance: currentBalance + refundAmount });
...
/* ----- View Submissions (loads counts + default tab onReview) ----- */
async function viewSubmissions(jobId, jobType) {
  showSection('jobSubmissionsSection');
  _currentViewingJob = { id: jobId, type: jobType, jobDoc: null }; // jobDoc will be loaded if needed

  // populate header and counts
  const headerEl = document.getElementById('submissionsHeader');
  headerEl.innerHTML = `<div class="text-sm text-gray-600">Loading job info...</div>`;

  try {
    const collection = jobType === 'task' ? 'tasks' : 'affiliateJobs';
    const doc = await db.collection(collection).doc(jobId).get();
    if (!doc.exists) headerEl.innerHTML = `<div class="text-red-500">Job not found.</div>`;
    else {
      _currentViewingJob.jobDoc = { id: doc.id, ...doc.data() };
      headerEl.innerHTML = `
        <div class="flex items-start justify-between">
          <div>
            <h3 class="text-lg font-bold">${escapeHtml(doc.data().title || 'Untitled Job')}</h3>
            <p class="text-sm text-gray-500">${jobType === 'task' ? 'Task submissions' : 'Affiliate submissions'}</p>
          </div>
          <div class="text-sm text-gray-600">Job ID: ${doc.id}</div>
        </div>
      `;
    }
  } catch (err) {
    console.error("viewSubmissions header err:", err);
  }
...
async function refreshSubmissionCounts() {
  if (!_currentViewingJob) return;
  const jobId = _currentViewingJob.id;
  const type = _currentViewingJob.type;
  const subCollection = type === 'task' ? 'task_submissions' : 'affiliate_submissions';
  try {
    const onReviewSnap = await db.collection(subCollection)
      .where(type === 'task' ? 'taskId' : 'jobId', '==', jobId)
      .where('status','==','on review')
      .get();
    const completedSnap = await db.collection(subCollection)
      .where(type === 'task' ? 'taskId' : 'jobId', '==', jobId)
...
/* ----- Switch submission tab & load submissions ----- */
async function switchSubmissionTab(tab) {
  _currentSubTab = tab;
  const list = document.getElementById('submissionsList');
  list.innerHTML = `<div class="text-center text-gray-500">Loading ${tab === 'onReview' ? 'on-review' : 'completed'} submissions...</div>`;
  if (!_currentViewingJob) { list.innerHTML = `<div class="text-red-500">No job selected.</div>`; return; }
  ...
  try {
    ...
    else q = q.where('status','in',['approved','rejected']).orderBy(type === 'task' ? 'submittedAt' : 'createdAt','desc');
    ...
    // find whether current user is the job owner to show approve/reject
    const currentUser = firebase.auth().currentUser;
    const ownerUid = _currentViewingJob.jobDoc?.postedBy?.uid || _currentViewingJob.jobDoc?.postedByUid || _currentViewingJob.jobDoc?.ownerUid || _currentViewingJob.jobDoc?.ownerId;
...
/* ----- Renderers for submission cards ----- */
function renderTaskSubmissionCard(s, isOwner) {
  ...
  const proofText = s.proofText ? `<p class="text-sm text-gray-700 mt-2">${escapeHtml(s.proofText)}</p>` : '';
  const images = Array.isArray(s.proofImages) ? s.proofImages.map(u => `<img src="${u}" class="w-28 h-20 object-cover rounded mr-2"/>`).join('') : '';
  const statusBadge = s.status === 'approved' ? 'text-green-600' : s.status === 'rejected' ? 'text-red-600' : 'text-yellow-600';

  const approveBtn = isOwner && s.status === 'on review' ? `<button class="job-action-btn primary" onclick="approveSubmission('${s.id}','${_currentViewingJob.id}','${_currentViewingJob.type}')"><i data-lucide="check" class="w-4 h-4"></i> Approve</button>` : '';
  const rejectBtn  = isOwner && s.status === 'on review' ? `<button class="job-action-btn secondary" onclick="rejectSubmission('${s.id}','${_currentViewingJob.id}','${_currentViewingJob.type}')"><i data-lucide="x" class="w-4 h-4"></i> Reject</button>` : '';

  return `
    <div class="p-4 rounded-lg border border-gray-100 bg-white shadow-sm">
      <div class="flex justify-between items-start">
        <div>
          <div class="text-sm text-gray-500">User: <span class="font-semibold">${escapeHtml(s.userName || s.userId || s.userUid || 'Unknown')}</span></div>
          <div class="text-xs text-gray-400">${time}</div>
        </div>
        <div class="${statusBadge} font-bold">${escapeHtml(s.status)}</div>
      </div>

      ${proofText}
      <div class="flex gap-2 mt-3">${images}</div>
...
function renderAffiliateSubmissionCard(s, isOwner) {
  ...
  const note = s.note ? `<p class="text-sm text-gray-700 mt-2">${escapeHtml(s.note)}</p>` : '';
  const files = Array.isArray(s.proofFiles) ? s.proofFiles.map(u => `<a href="${u}" target="_blank" class="block text-xs underline">${escapeHtml(u.split('/').pop() || u)}</a>`).join('') : '';
  const statusBadge = s.status === 'approved' ? 'text-green-600' : s.status === 'rejected' ? 'text-red-600' : 'text-yellow-600';
  ...
  return `
    <div class="p-4 rounded-lg border border-gray-100 bg-white shadow-sm">
      <div class="flex justify-between items-start">
        <div>
          <div class="text-sm text-gray-500">User: <span class="font-semibold">${escapeHtml(s.userName || s.userId || 'Unknown')}</span></div>
          <div class="text-xs text-gray-400">${time}</div>
        </div>
        <div class="${statusBadge} font-bold">${escapeHtml(s.status)}</div>
      </div>
...
/* ----- Approve submission (transaction: credit worker) ----- */
async function approveSubmission(submissionId, jobId, jobType) {
  if (!confirm("Approve this submission and credit the worker?")) return;
  ...
  try {
    await db.runTransaction(async tx => {
      ...
      if ((submission.status || '').toLowerCase() !== 'on review') throw new Error("Submission is no longer on review");

      const jobDoc = await tx.get(jobRef);
      if (!jobDoc.exists) throw new Error("Job not found");
      const job = jobDoc.data();

      // determine worker and amount
      const workerUid = submission.userId || submission.userUid || submission.user;
      if (!workerUid) throw new Error("Worker UID missing on submission");
      const amount = Number(submission.workerEarn || submission.workerPay || job.workerEarn || job.workerPay || 0);
      ...
      if (amount > 0) {
        ...
        const prev = Number(workerDoc.data().balance || 0);
        tx.update(workerRef, { balance: prev + amount });
      }
    });

    alert("Submission approved and worker credited (if amount > 0).");
    await switchSubmissionTab(_currentSubTab);
  } catch (err) {
...
/* ----- Reject submission (no money ops) ----- */
...
/* ----- Boot: expose init for your tabs ----- */
window.initMyJobsSection = initMyJobsSection;

/* Optional: auto-init when page loads and user already authenticated */
document.addEventListener('DOMContentLoaded', () => {
  // If user already logged in, init will show jobs later when you call initMyJobsSection()
  if (window.lucide && lucide.createIcons) lucide.createIcons();
});
