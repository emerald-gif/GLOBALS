...
// AFFILIATE - single-file script (admin swiper removed)
(function () {
...
async function loadFinishedTasks() {
  ...
  try {
    ...
    let pending = 0, approved = 0;
    if (!docs.length) {
      if (listEl) listEl.innerHTML = `<p class="text-center text-gray-500">No finished tasks yet.</p>`;
    }
    for (const data of docs) {
      if (data.status === 'on review') pending++;
      if (data.status === 'approved') approved++;
...
// Show submission details (fetch from affiliateJobs)
