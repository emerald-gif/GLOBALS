...
(() => {
  ...
  function createCardElement(tx) {
    ...
    // Build subtitle line based on type
    let subtitle = '';
    if (type === 'airtime')     subtitle = (tx.network||'') + (tx.phone ? ' · ' + tx.phone : '');
    else if (type === 'data')   subtitle = (tx.network||'') + (tx.phone ? ' · ' + tx.phone : '') + (tx.planLabel ? ' · ' + tx.planLabel : '');
    else if (type === 'electricity') subtitle = (tx.distro||tx.serviceID||'') + (tx.meterNumber ? ' · ' + tx.meterNumber : '');
    else if (type === 'tv')     subtitle = (tx.provider||tx.serviceID||'') + (tx.smartcard ? ' · ' + tx.smartcard : '') + (tx.planLabel ? ' · ' + tx.planLabel : '');
    else if (type === 'betting') subtitle = (tx.platform||tx.serviceID||'') + (tx.customerID ? ' · ' + tx.customerID : '');
    else if (type === 'education') subtitle = (tx.examType||tx.serviceID||'') + (tx.phone ? ' · ' + tx.phone : '') + (tx.planLabel ? ' · ' + tx.planLabel : '');
    else if (type === 'withdraw') subtitle = (tx.bankName||'') + (tx.accNum ? ' · ' + tx.accNum : '');
    else if (type === 'deposit') subtitle = '';

    // Icon emoji per type
    const iconMap = { airtime:'📱', data:'📶', electricity:'⚡', tv:'📺', betting:'🎰', education:'🎓', withdraw:'🏦', deposit:'💰' };
    ...
    wrapper.innerHTML = `
      <div style="display:flex;align-items:center;gap:12px;">
        <div style="width:42px;height:42px;border-radius:14px;background:#f4f6fb;display:flex;align-items:center;justify-content:center;font-size:1.3rem;flex-shrink:0;">${icon}</div>
        <div style="flex:1;min-width:0;">
          <p style="font-family:'Sora',sans-serif;font-size:0.88rem;font-weight:700;color:#0d1b2e;margin:0;">${tx.type || "Transaction"}</p>
          ${subtitle ? `<p style="font-family:'DM Sans',sans-serif;font-size:0.72rem;color:#8a96a8;margin:2px 0 0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${subtitle}</p>` : ''}
          <p style="font-family:'DM Sans',sans-serif;font-size:0.7rem;color:#b0b8c8;margin:2px 0 0;">${formatDatePretty(ts)}</p>
        </div>
        <div style="text-align:right;flex-shrink:0;">
          <p style="font-family:'Sora',sans-serif;font-size:0.95rem;font-weight:700;margin:0;" class="${amountClass}">${formatAmount(tx.amount)}</p>
          <span style="display:inline-block;margin-top:3px;padding:2px 8px;border-radius:20px;font-size:0.65rem;font-weight:600;font-family:'DM Sans',sans-serif;${
            tx.status==='success'||tx.status==='successful' ? 'background:#dcfce7;color:#16a34a;' :
  ...
  try {
    ...
    snapshot.forEach(doc => {
      ...
      if (notifList) {
        ...
        msgEl.style.cssText = 'font-size:0.85rem;color:#4a5568;margin:0;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden;line-height:1.5;';
        msgEl.textContent = message;
        ...
        viewBtn.innerHTML = 'View <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M9 18l6-6-6-6"/></svg>';
        viewBtn.onclick = (e) => {
          e.stopPropagation();
          const titleEl2 = document.getElementById('notifDetailTitle');
          const msgEl2 = document.getElementById('notifDetailMessage');
          const dateEl2 = document.getElementById('notifDetailDate');
          if (titleEl2) titleEl2.textContent = title;
          if (msgEl2) msgEl2.textContent = message;
...
(() => {
  ...
  async function elecPay() {
    ...
    try {
      ...
      const data = await resp.json();
      if (!resp.ok || data.status==='fail') { showErr('ec-error', data.message||'Failed. Try again.'); btn.disabled=false; btn.textContent=orig; return; }
    ...
    } catch(e) { showErr('ec-error','Network error. Try again.'); btn.disabled=false; btn.textContent=orig; }
  }
...
(() => {
  ...
  async function tvPay() {
    ...
    try {
      ...
      const data = await resp.json();
      if (!resp.ok||data.status==='fail') { showErr('tv-error-confirm',data.message||'Failed. Try again.'); btn.disabled=false; btn.textContent=orig; return; }
    ...
    } catch(e) { showErr('tv-error-confirm','Network error. Try again.'); btn.disabled=false; btn.textContent=orig; }
  }
...
(() => {
  ...
  const BET_ICONS = {'bet9ja':'🎰','sportybet':'⚽','betway-ng':'🏆','1xbet':'🎯','betking':'👑','nairabet':'💚','merrybet':'🎲','parimatch':'🎽','accessbet':'🏅'};
  const BET_LABELS = {'bet9ja':'Bet9ja','sportybet':'SportyBet','betway-ng':'Betway','1xbet':'1xBet','betking':'BetKing','nairabet':'NairaBet','merrybet':'MerryBet','parimatch':'PariMatch','accessbet':'AccessBet'};
  ...
  function bettingOpen(){_show('bet-screen');window._renderRecentChips('betting','bet-customerid','bet-recent');}
  ...
  async function betPay(){
    ...
    try{
      ...
      const data=await resp.json();
      if(!resp.ok||data.status==='fail'){showErr('bet-error-confirm',data.message||'Failed. Try again.');btn.disabled=false;btn.textContent=orig;return;}
    ...
    }catch(e){showErr('bet-error-confirm','Network error. Try again.');btn.disabled=false;btn.textContent=orig;}
  }
...
(() => {
  ...
  function educationOpen(){_show('edu-screen');window._renderRecentChips('education','edu-phone','edu-recent');}
  ...
  async function eduPay(){
    ...
    try{
      ...
      const data=await resp.json();
      if(!resp.ok||data.status==='fail'){showErr('edu-error-confirm',data.message||'Failed. Try again.');btn.disabled=false;btn.textContent=orig;return;}
    ...
    }catch(e){showErr('edu-error-confirm','Network error. Try again.');btn.disabled=false;btn.textContent=orig;}
  }
