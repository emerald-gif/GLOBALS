...
async function downloadReceiptPDF(tx) {
  ...
  // Load logo
  const logoUrl = 'https://res.cloudinary.com/dyquovrg3/image/upload/v1770534119/wcl6sd2jl7tzwgnk4sal.png';
...
async function shareReceiptImage(tx) {
  const card = document.querySelector("#transaction-details-content .bg-white"); // select card
  if (!card) return;

  // Add logo to card temporarily
  const logoUrl = 'https://res.cloudinary.com/dyquovrg3/image/upload/v1770534119/wcl6sd2jl7tzwgnk4sal.png';
...
document.getElementById("download-pdf-btn").onclick = () => downloadReceiptPDF(tx);
document.getElementById("share-btn").onclick = () => shareReceiptImage(tx);
