...
(() => {
  ...
  async function tvSelectProvider(id) {
    tvProvider = id; tvSelectedPlan = null;
  ...
  window._openAirtimeNetSheet = function() {
    window.openBottomSheet({
      ...
      onSelect: function(item) {
        ...
        if (imgEl) imgEl.innerHTML = '<img src="'+item.img+'" style="width:100%;height:100%;border-radius:10px;object-fit:contain;" onerror="this.style.display=\'none\'">';
      }
    });
  };
...
// ── Missing global helpers ────────────────────────────────────────────────────

// Close the notification popup (used by ✕ button in notif banner)
window.closeNotifPopup = function () {
  const el = document.getElementById('notifPopup');
  if (el) el.classList.add('hidden');
};

// Language selection (UI only — stores preference in localStorage)
window.selectLanguage = function (lang) {
  localStorage.setItem('globals_language', lang);
  // Highlight active option
  document.querySelectorAll('.language-option').forEach(btn => {
    btn.classList.toggle('active', btn.textContent.includes(lang));
  });
  // Visual feedback
  const toast = document.createElement('div');
  toast.textContent = 'Language set to ' + lang;
  toast.style.cssText = 'position:fixed;bottom:80px;left:50%;transform:translateX(-50%);background:#1a6fff;color:#fff;padding:10px 22px;border-radius:30px;font-family:Sora,sans-serif;font-size:0.85rem;font-weight:600;z-index:9999;opacity:1;transition:opacity 0.4s;';
