...
(() => {
  ...
  function airtimeOpen(){
    ...
    // Attach recent chips
    window._renderRecentChips('airtime', 'airtime-phone', 'airtime-recent');
    window._loadServiceHistory('Airtime','airtime-history',v=>{
      const inp=document.getElementById('airtime-phone'); if(inp) inp.value=v;
    });
  }
  ...
  async function airtimePay(){
    ...
    try {
      ...
      _airtimePaying=false; window.btnLoad(btn, false, 'Pay Now');
      // Save recent
      window._saveRecent('airtime', phone, phone);
...
(() => {
  ...
  function dataOpen(){
    ...
    window._renderRecentChips('data','data-phone','data-recent');
    window._loadServiceHistory('Data','data-history',v=>{
      const inp=document.getElementById('data-phone'); if(inp) inp.value=v;
    });
  ...
  async function dataPay(){
    ...
    try{
      ...
      _dataPaying=false; window.btnLoad(btn,false,'Pay Now');
      window._saveRecent('data', phone, phone);
...
(function() {
  ...
  window._saveRecent = function(service, value, label) {
    if (!value) return;
    try {
      const key = 'globals_recent_' + service;
      let arr = JSON.parse(localStorage.getItem(key) || '[]');
      // Remove if already exists
      arr = arr.filter(x => x.value !== value);
      arr.unshift({ value, label: label || value });
  ...
  window._renderRecentChips = function(service, inputId, containerId, labelFn) {
    const container = document.getElementById(containerId);
    if (!container) return;
    const recents = window._getRecent(service);
    if (!recents.length) { container.style.display = 'none'; return; }
    container.style.display = 'flex';
    container.innerHTML = recents.map(r =>
      `<button class="recent-chip" onclick="document.getElementById('${inputId}').value='${r.value}';${labelFn||''}">${r.label}</button>`
    ).join('');
  };
})();
...
(() => {
  ...
  function electricityOpen() {
    _show('elec-screen');
    window._renderRecentChips('electricity', 'elec-meter', 'elec-recent');
    window._loadServiceHistory('Electricity', 'elec-history', v => {
      document.getElementById('elec-meter').value = v;
      elecClearVerify();
  ...
  async function elecPay() {
    ...
    try {
      ...
      // Save to recent
      window._saveRecent('electricity', c.dataset.meter, c.dataset.meter);
...
(() => {
  ...
  function tvOpen() {
    _show('tv-screen');
    window._renderRecentChips('tv','tv-smartcard','tv-recent');
    window._loadServiceHistory('TV', 'tv-history', v => {
      document.getElementById('tv-smartcard').value = v;
      tvClearVerify();
      // Auto-verify if provider already selected
  ...
  async function tvSelectProvider(id) {
    ...
    // Load plans
    if (_plansCache[id]) { _renderTvPlans(_plansCache[id]); return; }
    ...
    try {
      ...
      const data = await resp.json();
      _plansCache[id] = data.plans || [];
      _renderTvPlans(_plansCache[id]);
    } catch(e) { if(plansWrap) plansWrap.innerHTML='<p style="font-size:0.78rem;color:#ef4444;padding:8px 0;font-family:\'DM Sans\',sans-serif;">Could not load plans.</p>'; }
  }
  ...
  async function tvPay() {
    ...
    try {
      ...
      window._saveRecent('tv', c.dataset.smartcard, c.dataset.smartcard);
...
(() => {
  ...
  function bettingOpen(){
    _show('bet-screen');
    window._renderRecentChips('betting','bet-customerid','bet-recent');
    window._loadServiceHistory('Betting','bet-history',v=>{document.getElementById('bet-customerid').value=v;});
  }
  ...
  async function betPay(){
    ...
    try{
      ...
      _betPaying=false;
      window._saveRecent('betting', c.dataset.customerID, BET_LABELS[c.dataset.serviceID]+' · '+c.dataset.customerID);
...
(() => {
  ...
  function educationOpen(){
    _show('edu-screen');
    window._renderRecentChips('education','edu-phone','edu-recent');
    window._loadServiceHistory('Education','edu-history',v=>{document.getElementById('edu-phone').value=v;});
  }
  ...
  async function eduSelectProvider(id){
    ...
    if(_plansCache[id]){_renderEduPlans(_plansCache[id]);return;}
    ...
    try{
      ...
      const data=await resp.json();
      _plansCache[id]=data.plans||[];
      _renderEduPlans(_plansCache[id]);
      // Show qty for WAEC (can buy multiple scratch cards)
    ...
    }catch(e){if(plansWrap)plansWrap.innerHTML='<p style="font-size:0.78rem;color:#ef4444;padding:8px 0;font-family:\'DM Sans\',sans-serif;">Could not load products.</p>';}
  }
    ...
    try{
      ...
      const resp=await fetch('/api/vtu/education',{
        ...
        body:JSON.stringify({
          ...
          phone:c.dataset.phone, quantity:c.dataset.qty,
      ...
      _eduPaying=false;
      window._saveRecent('education',c.dataset.phone,c.dataset.phone);
...
(function() {
  ...
  /* ─── 5. Service history loader ─── */
  window._loadServiceHistory = async function(type, containerId, fillFields) {
    const container = document.getElementById(containerId);
    if (!container) return;
    ...
    try {
      container.innerHTML = '<p style="font-family:\'DM Sans\',sans-serif;font-size:0.75rem;color:#b0b8c8;padding:4px 0;">Loading history…</p>';
      const snap = await db.collection('Transaction')
      ...
      const statusColor = s => s==='success'||s==='successful'?'#16a34a':s==='failed'?'#dc2626':'#b45309';
      const statusBg    = s => s==='success'||s==='successful'?'#dcfce7':s==='failed'?'#fee2e2':'#fef9c3';

      container.innerHTML = `
        <p style="font-family:'Sora',sans-serif;font-size:0.78rem;font-weight:700;color:#4a5568;margin:0 0 8px;">Recent</p>
        ${items.map(tx => {
          const sub = tx.phone||tx.meterNumber||tx.smartcard||tx.customerID||'';
          const plan = tx.planLabel||tx.distro||tx.provider||tx.platform||tx.examType||'';
          return `<div onclick="window._historyFill(${JSON.stringify(sub).replace(/"/g,"'")})" style="display:flex;align-items:center;justify-content:space-between;padding:9px 12px;background:#f9fafc;border-radius:12px;margin-bottom:6px;cursor:pointer;border:1px solid #e4e8f0;">
            <div>
              <p style="font-family:'DM Sans',sans-serif;font-size:0.8rem;font-weight:600;color:#0d1b2e;margin:0;">${sub||plan||'—'}</p>
              <p style="font-family:'DM Sans',sans-serif;font-size:0.7rem;color:#8a96a8;margin:2px 0 0;">₦${Number(tx.amount||0).toLocaleString()} · ${plan}</p>
            </div>
            <span style="font-size:0.65rem;font-weight:600;padding:2px 8px;border-radius:20px;background:${statusBg(tx.status)};color:${statusColor(tx.status)};">${tx.status==='success'?'✓':tx.status}</span>
          </div>`;
        }).join('')}
      `;

      window._historyFill = function(val) {
        if (typeof fillFields === 'function') fillFields(val);
      };
      ...
      if (!_tabLoadState[tab]) {
        ...
        switch(tab) {
          case 'referral-section':
            ...
            break;
          case 'notifications':
            if (typeof window.fetchNotificationsOnce === 'function') window.fetchNotificationsOnce();
            break;
          case 'transactions-screen':
