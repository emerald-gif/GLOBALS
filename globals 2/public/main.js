...
function renderAdminSwiper(jobs) {
  ...
  if (!jobs.length) {
    slidesContainer.innerHTML = `
      <div class="swiper-slide">
        <div class="rounded-2xl p-6 bg-white shadow">No featured jobs</div>
      </div>
    `;
  }
    ...
    const slideHtml = `
      <div class="swiper-slide px-2">
        <div class="rounded-2xl overflow-hidden shadow-lg bg-white">
          <img src="${escapeHtml(img)}" alt="${escapeHtml(job.title)}" class="w-full h-56 object-cover">
          <div class="p-4">
            ...
            <button class="mt-4 w-full py-3 rounded-xl bg-blue-600 text-white view-admin-job" 
                    data-id="${job.id}" data-type="${job.jobType || 'admin'}">View</button>
          </div>
        </div>
      </div>
    `;
  ...
  window.setTimeout(() => { // small timeout to ensure DOM updated
...
function renderAffiliateGrid(jobs) {
  ...
  if (!jobs.length) {
    grid.innerHTML = `<div class="col-span-full p-6 bg-white rounded-xl shadow">No affiliate tasks currently.</div>`;
    return;
  }

  jobs.forEach(job => {
    // calculate taken / progress if there are worker arrays; fallback to 0
    const taken = job.takenWorkers || (job.workersAssigned ? job.workersAssigned.length : 0) || 0;
    ...
    const card = `
      <div class="bg-white rounded-2xl shadow p-4">
        ...
        <div class="flex items-center gap-3 mt-3">
          <img src="${escapeHtml(img)}" class="w-12 h-12 rounded-full object-cover" />
          <div class="flex-1">
            <h4 class="font-semibold text-md">${escapeHtml(job.title)}</h4>
            <div class="text-sm text-gray-500 mt-1">${formatNaira(job.workerPay)} · ${taken}/${total} workers</div>
          </div>
        </div>
        <div class="mt-4">
          <div class="w-full bg-gray-100 rounded-full h-2 overflow-hidden">
            <div style="width:${percent}%;" class="h-2 rounded-full bg-blue-300"></div>
          </div>
          <button class="mt-4 w-full py-3 rounded-xl bg-blue-600 text-white view-affiliate-task" 
                  data-id="${job.id}" data-type="${job.jobType || 'affiliate'}">View Task</button>
        </div>
      </div>
    `;
...
/* --------- Real-time listeners --------- */
function listenToAdminJobsSwiper() {
  // show admin posted jobs as featured slides (status: "active")
  db.collection("adminJobs").where("status", "==", "active")
    .onSnapshot(snapshot => {
      const jobs = [];
      snapshot.forEach(doc => {
        const data = doc.data(); data.id = doc.id; jobs.push(data);
      });
      // optional: sort by postedAt desc
      jobs.sort((a,b) => (b.postedAt?.seconds || 0) - (a.postedAt?.seconds || 0));
      renderAdminSwiper(jobs);
    }, err => console.error("adminJobs snapshot err:", err));
}

function listenToApprovedAffiliateJobs() {
  // show affiliate jobs that were approved by admin (status: "approved")
  db.collection("affiliateJobs").where("status", "==", "approved")
    ...
    }, err => console.error("affiliateJobs snapshot err:", err));
}

/* --------- Init function to call on dashboard load --------- */
function initJobsUIOnDashboard() {
  listenToAdminJobsSwiper();
  listenToApprovedAffiliateJobs();

  // Delegated click handlers for View buttons (activate later)
  document.addEventListener("click", (e) => {
    const adminBtn = e.target.closest(".view-admin-job");
    if (adminBtn) {
      const id = adminBtn.dataset.id;
      console.log("View admin job clicked:", id);
      // later: open modal or navigate to job detail
      return;
    }
    const affBtn = e.target.closest(".view-affiliate-task");
    if (affBtn) {
      const id = affBtn.dataset.id;
      console.log("View affiliate task clicked:", id);
      // later: open modal or navigate to task detail
      return;
    }
  });
}

// call this in your dashboard init after firebase and UI are ready:
initJobsUIOnDashboard();
